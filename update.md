...
82. **2026-01-31**:
    - **BUG修复**: 优化 CSV 导出逻辑，现在导出的数据包含日期、时间及所有维度分列，完全符合导入规范。 (修改人: Antigravity)
    - **功能改进**: 坐标轴设置优化，手动调整（键盘上下键）现在以当前数值或数据默认值为基准开始，不再强制从 0 开始。 (修改人: Antigravity)
    - **功能改进**: 实现坐标轴负数自适应，当数据包含负值时，坐标轴下限自动调整为最小值 110%。 (修改人: Antigravity)
